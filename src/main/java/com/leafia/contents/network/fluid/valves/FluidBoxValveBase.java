...
abstract public class FluidBoxValveBase extends BlockContainer implements ILookOverlay {
	public static final PropertyBool STATE = PropertyBool.create("state");
	public FluidBoxValveBase(Material materialIn, String s) {
		super(materialIn);
	...
	@Nullable
	public IBlockState getValveBlockState(World world,BlockPos pos) {
		if (world.isValid(pos)) {
			IBlockState state = world.getBlockState(pos);
			if (state.getBlock() instanceof FluidBoxValveBase)
				return state;
		}
		return null;
	}
	public boolean getState(World world,BlockPos pos) {
		IBlockState state = getValveBlockState(world,pos);
		if (state != null)
			return state.getValue(STATE);
		return false;
	}
	public void setState(World world,BlockPos pos,boolean state) {
		IBlockState istate = getValveBlockState(world,pos);
		if (istate == null || getState(world,pos) == state) return;

		TileEntity te = world.getTileEntity(pos);
		if (!(te instanceof FluidBoxValveTE)) return;
		FluidBoxValveTE valve = (FluidBoxValveTE)te;
		Fluid type = valve.getType();
		world.setBlockState(pos,istate.withProperty(STATE,state));

		TileEntity nte = world.getTileEntity(pos);
		if (!(nte instanceof FluidBoxValveTE)) { FluidBoxValveTE.rebuildNetworks(world,pos); return; }
		FluidBoxValveTE nvalve = (FluidBoxValveTE)nte;
		nvalve.setType(type);
	}
	...
	public IBlockState getStateFromMeta(int meta) {
		return meta > 0 ? this.getDefaultState().withProperty(STATE, true) : this.getDefaultState().withProperty(STATE, false);
	}
