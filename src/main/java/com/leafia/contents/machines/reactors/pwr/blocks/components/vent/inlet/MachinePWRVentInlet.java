...
public class MachinePWRVentInlet extends MachinePWRVentBase {
	public MachinePWRVentInlet() {
	...
	public TileEntity createNewTileEntity(World worldIn,int meta) {
		return new TileEntityPWRVentInlet();
	}
	...
	public boolean correctDirection(World world,BlockPos pos) {
		...
		while (world.isValid(pos)) {
			...
			if (state2.getBlock() instanceof MachinePWRVentOutlet) {
	...
	public void onBlockPlacedBy(World world,BlockPos pos,IBlockState state,EntityLivingBase placer,ItemStack stack) {
		...
		if (world.isValid(pos.offset(face.getOpposite()))) {
			...
			if (!detect.getMaterial().isReplaceable()) {
				if (detect.getBlock() instanceof MachinePWRVentElement)
		...
		if (te instanceof TileEntityPWRVentInlet) {
	...
	public void neighborChanged(IBlockState state,World worldIn,BlockPos pos,Block blockIn,BlockPos fromPos) {
		if (!(worldIn.getBlockState(pos.offset(state.getValue(MachinePWRVentBase.FACING).getOpposite())).getBlock() instanceof MachinePWRVentDuct))
