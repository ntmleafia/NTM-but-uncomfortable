...
public class CraftingPWRDebris extends net.minecraftforge.registries.IForgeRegistryEntry.Impl<IRecipe> implements IRecipe {
	@Override
	public boolean matches(InventoryCrafting inv,World worldIn) {
		if (inv.getSizeInventory() == 9) {
			...
			if (oneStack.getItem() instanceof ItemPWRDebris) {
				return (((ItemPWRDebris) oneStack.getItem()).canBeCraftedBack) && oneStack.getTagCompound() != null;
			}
		}
		return false;
	}
	...
	public ItemStack getCraftingResult(InventoryCrafting inv) {
		...
		if (!stk.isEmpty()) {
			...
			if (nbt != null && stk.getItem() instanceof ItemPWRDebris) {
				ItemPWRDebris debrisItem = (ItemPWRDebris)stk.getItem();
