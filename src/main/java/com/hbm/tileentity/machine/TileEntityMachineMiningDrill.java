	...
	public void update() {
		if(!world.isRemote) {
			...
			for(int i = 10; i < 13; i++) {
				...
				if(stack != null) {
					if(stack.getItem() == Upgrades.upgrade_effect_1) {
						this.radius += 1;
						this.consumption += 80;
					}
					if(stack.getItem() == Upgrades.upgrade_effect_2) {
						this.radius += 2;
						this.consumption += 160;
					}
					if(stack.getItem() == Upgrades.upgrade_effect_3) {
						this.radius += 3;
					...
					if(stack.getItem() == Upgrades.upgrade_fortune_1) {
						this.fortune += 1;
						this.timer += 15;
					}
					if(stack.getItem() == Upgrades.upgrade_fortune_2) {
						this.fortune += 2;
						this.timer += 30;
					}
					if(stack.getItem() == Upgrades.upgrade_fortune_3) {
						this.fortune += 3;
						this.timer += 45;
					}
				}
