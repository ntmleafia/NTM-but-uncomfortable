...
public class TileEntityCore extends TileEntityMachineBase implements ITickable, LeafiaPacketReceiver {
	public enum Cores {
		...
		Cores(SoundEvent sfx,BiFunction<Float,Double,Double> attentuationFunction) { this.sfx = sfx; this.attentuationFunction = attentuationFunction; }
	}
	...
	public void update() {
		...
		if(!world.isRemote) {
			...
			if(catalystA.getItem() instanceof ItemCatalyst && catalystB.getItem() instanceof ItemCatalyst){
				...
				if (tagA == null) { tagA = new NBTTagCompound(); catalystA.setTagCompound(tagA); }
				if (tagB == null) { tagB = new NBTTagCompound(); catalystB.setTagCompound(tagB); }
			...
			*/
			...
			Integer coreId = null;
			try {
				coreId = Cores.valueOf(inventory.getStackInSlot(1).getItem().getRegistryName().getResourcePath()).ordinal();
			} catch (IllegalArgumentException | NullPointerException ignored) {} // fuck you im lazy
			LeafiaPacket._start(this)
