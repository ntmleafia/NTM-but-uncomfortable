    ...
    private void updateCollections(boolean regenerateTabs)
    {
        ...
        /*if (regenerateTabs)*/ {
            ...
            for (int i = 0; i < filtered_recipes.size() - page * 20; i++) {
                ...
                Item item = why.getKey().getKey();
                int amount = why.getKey().getValue();
                List<LeafiaDummyRecipe> recipes = why.getValue();
    ...
    public Map<ItemStack,Map<Integer,Integer>> getSlotsToTakeFrom(AStack item,Map<Integer,Integer> replacementMap,Set<Integer> blockedSlots) {
        ...
        if (item instanceof OreDictStack) {
            ...
            for (Pair<Item,Integer> pair : most) {
                ...
                boolean doTry = true;
                while (doTry) {
                    int dec = Math.max(required - pair.getKey().getItemStackLimit(),0);
                    Pair<Map<Integer,Integer>,Integer> picks = pickItem(pair.getKey(),required - dec,replacementMap,blockedSlots);
                    int taken = (required - dec) - picks.getValue();
                    required = picks.getValue() + dec;
                    for (Integer slotIndex : picks.getKey().keySet()) {
                        if (slotsTake.containsKey(slotIndex))
                            slotsTake.put(slotIndex,slotsTake.get(slotIndex) + picks.getKey().get(slotIndex));
                        else
                            slotsTake.put(slotIndex,picks.getKey().get(slotIndex));
                    }
                    doTry = false;
                    if (slotsTake.size() > 0) {
                        itemsTake.put(new ItemStack(pair.getKey(),taken),slotsTake);
                        if (dec > 0)
                            doTry = true;
                    }
                    if (required <= 0) break;
                    ...
                    if (recipe != null) {
                        ...
                        if (mouseButton == 0) {
                            ...
                            if (container.getSlot(slotIndex).getHasStack())
                                ...
                                for (Integer slotDecrease : slotTake.getValue().keySet()) {
                                    //container.getSlot(slotDecrease).decrStackSize(slotTake.getValue().get(slotDecrease));
                                    packet.transfer.add(new Tuple.Triplet<>(slotDecrease,slotTake.getValue().get(slotDecrease),slotIndex));
                                }
                            }
