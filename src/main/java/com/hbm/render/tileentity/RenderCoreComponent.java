...
public class RenderCoreComponent extends TileEntitySpecialRenderer<TileEntityMachineBase> {
	...
	public void render(TileEntityMachineBase tileEntity, double x, double y, double z, float partialTicks, int destroyStage, float alpha) {
		...
		if (te.lastGetCore != null)
			range = new Vec3d(te.getPos()).addVector(0.5,0.5,0.5).distanceTo(new Vec3d(te.lastGetCore.getPos()).addVector(0.5,0.5,0.5));
        ...
        if(tileEntity instanceof TileEntityCoreEmitter) {
			...
			if (result != null) {
				range = new Vec3d(te.getPos()).addVector(0.5,0.5,0.5).distanceTo(result.hitVec);
		...
		for (EnumFacing face : EnumFacing.values()) {
			...
			if (isFront) {
				for (int i = 0; i < 4; i++) {
					if (face == EnumFacing.UP) {
						if (!isFace(EnumFacing.getHorizontal(Math.floorMod(-i,4)),unit))
							mdl.renderPart("Arm");
					} else if (face == EnumFacing.DOWN) {
						if (!isFace(EnumFacing.getHorizontal(Math.floorMod(2-i,4)),unit))
							mdl.renderPart("Arm");
					} else {
						EnumFacing check;
